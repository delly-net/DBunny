...
在 DBunny 项目中实现 LINQ 支持，允许用户使用 LINQ 语法进行数据库查询操作，同时保持与现有 DBunny API 的兼容性，并通过代码改造实现 AOT 编译兼容。
...
namespace Delly.DBunny.Linq.Compiler
{
    ...
    internal sealed class ExpressionCompiler
    {
        ...
        private void CompileRoot(QueryRootExpression rootExpr, StringBuilder sqlBuilder)
        {
            ...
            var tableName = _expressionProvider.GetName(model.TableName);
...
1. **避免反射**：不使用 `Assembly.GetTypes()` 等反射 API
2. **显式编译**：通过 `ExpressionCompiler` 显式处理已知表达式类型
3. **编译时确定**：所有类型访问在编译时确定
4. **无 AOT 特性**：不使用 `DynamicallyAccessedMembers`、`UnconditionalSuppressMessage` 等特性
